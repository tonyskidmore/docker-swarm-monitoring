...
Param(
    ...
    $DeployList = "consul,rabbitmq,haproxy,javaapp,monitoring,publisher,consumer"
...
# turn DeployList into an array
$DeployServices = $DeployList.split(',')
...
if('haproxy' -in $DeployServices) {

    # deploy haproxy if not already deployed
    Write-Verbose -Message "Deploying HAProxy if not already deployed"
...
if('monitoring' -in $DeployServices) {
...
if('publisher' -in $DeployServices) {
